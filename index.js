...
function renderCourses() {
  ...
  selectedCourses.forEach(course => {
    courseListHTML += `<li>${course.name}: <input required type="text" id="${course.name}"  placeholder="${course.credits} credits" required  onchange="updateGrade('${course.name}', this.value)"></li>`;
  });
...
function updateSPI() {
  ...
  if (anyCourseHasNoGrade) {
    alert("Please Enter All Grades"); // Show alert only once if any course has no grade
}
